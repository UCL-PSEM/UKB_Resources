...
def interpolated_curve(flow, n): # Fourier interpolation of the flow curve
    ...
    return flowint , inflectiontime
    
def plot_inflectiontime(file_name, ascending, descending, n): 
    ...
    plt.grid(True)
    plt.show()
    print(file_name[:7]+"Intersection with x-axis (X) for ascending: ", x_point," and for descending: ",x_point2)
...
for file_name in mask_dic:
    ...
    plot_inflectiontime(file_name, ascending, descending, 900)
...
df.to_csv('Results/Flow.csv', index=False)
