...
def first_half_max_x(x, y, target=0.5): #after normalisation, max is y=1
...
def interpolated_curve(flow, n): 
    ...
    flowint=normalize_flow(flowint) #flowint is the normalised flow curve
    x = np.arange(0, len(flowint))  # or any appropriate domain
    half_max_time =first_half_max_x(x, flowint)
    return flowint , half_max_time
    
def plot_curves(file_name, ascending, descending, n): 
    ...
    plt.plot(x_point, 0, 'bo', color='yellow', markersize=5, label='Point (a, b)')# Plot the point (a, b)
    plt.plot(x_point2, 0, 'bo', color='cyan', markersize=5, label='Point (a, b)')# Plot the point (a, b)
    plt.axhline(0, color='k', linestyle='--', linewidth=2)# Plot the x-axis
    plt.axvline(x=x_point, color='r', linestyle='--', linewidth=1)
    plt.axvline(x=x_point2, color='r', linestyle='--', linewidth=1)
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend()
    plt.grid(True)
    plt.show()
    print(file_name[:7]+"Half-max time point for ascending: ", x_point," and for descending: ",x_point2)
...
for file_name in mask_dic:
    ...
    plot_curves(file_name, ascending, descending, 900)
...
df.to_csv('Results/Flow_halfmax.csv', index=False)
